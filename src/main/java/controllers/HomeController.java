...
import com.projetai.DomainSuggestionCV;
...
public class HomeController {
    ...
    public void initialize() {
        try {
            ...
            DomainSuggestionCV.init(); // Initialiser le modèle et les attributs
            System.out.println("✅ DomainSuggestionCV initialisé !");
    ...
    @FXML
    private void onSuggestClick(ActionEvent event) {
        ...
        try {
            // Prédiction du domaine
            String domaineSuggere = DomainSuggestionCV.predict(texte);
            txtResult.setText(domaineSuggere);

            // Ajouter le domaine au ComboBox s'il n'existe pas déjà
            if (!domaineComboBoxAI.getItems().contains(domaineSuggere)) {
                domaineComboBoxAI.getItems().add(domaineSuggere);
            }
