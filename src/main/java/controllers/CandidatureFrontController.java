...
public class CandidatureFrontController implements Initializable {
    ...
    private void applyFilters() {
        ...
        if (filtered.isEmpty()) {
            cardsFlowPane.getChildren().add(
                    buildEmptyState("📭", "Aucune candidature", "Ajoutez une candidature ou modifiez la recherche."));
        } else {
            filtered.forEach(c -> cardsFlowPane.getChildren().add(buildCard(c)));
        }
    }
    ...
    private VBox buildCard(Candidature c) {
        ...
        // Score — lecture seule
        Label scoreLabel = new Label(c.getScore() == null
                ? "En attente d'évaluation"
                : String.format("%.2f / 10", c.getScore()));
        ...
        btnQR.setStyle(
                "-fx-background-color: linear-gradient(from 0% 0% to 100% 0%, #4f62e8, #7c3aed);" +
                        "-fx-text-fill: white;" +
                        "-fx-font-size: 12px;" +
                        "-fx-font-weight: 700;" +
                        "-fx-padding: 7 14;" +
                        ...
                        "-fx-cursor: hand;" +
                        "-fx-effect: dropshadow(gaussian, rgba(79,98,232,0.4), 8, 0, 0, 2);"
        );
        btnQR.setTooltip(new Tooltip("Générer le QR Code de suivi de cette candidature"));
        btnQR.setOnAction(e -> ouvrirQRCode(c));

        Button btnHide = new Button("Masquer");
        btnHide.getStyleClass().add("btn-ghost-danger");
        btnHide.setOnAction(e -> {
            try {
                service.hideCandidature(c.getIdCandidature());
                all.remove(c);
                updateStats();
                applyFilters();
                if (appController != null) appController.refreshSidebarStats();
        ...
        footer.getChildren().addAll(btnQR, sp, btnHide, btnSave);
    ...
    /**
     * Ouvre le panneau QR Code pour la candidature sélectionnée.
     */
    private void ouvrirQRCode(Candidature candidature) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/fxml/QRCodePanel.fxml"));
            Parent root = loader.load();

            QRCodeController ctrl = loader.getController();
            ctrl.setCandidature(candidature);

            Stage stage = new Stage();
            stage.initModality(Modality.APPLICATION_MODAL);
            stage.initOwner(cardsFlowPane.getScene().getWindow());
            stage.setTitle("📱 QR Code  —  " + nvl(candidature.getNomCandidature()));
            ...
            e.printStackTrace();
            alert("Erreur QR", "Impossible d'ouvrir le panneau QR Code :\n" + e.getMessage(),
                    Alert.AlertType.ERROR);
        }
    }

    // ─── Sauvegarde ──────────────────────────────────────────────────────────
    private void handleSave(Candidature c, TextField tfNom, TextField tfStartup,
                            DatePicker dp, TextArea ta, Button btn) {
        if (tfNom.getText().trim().isEmpty()) {
            alert("Requis", "Nom obligatoire.", Alert.AlertType.WARNING); return;
        }
        if (tfStartup.getText().trim().isEmpty()) {
            alert("Requis", "Startup obligatoire.", Alert.AlertType.WARNING); return;
        }
        ...
        try {
            ...
            new Thread(() -> {
                ...
                Platform.runLater(() -> { btn.setText("💾  Sauvegarder"); btn.setDisable(false); });
            }).start();
        } catch (SQLException ex) {
            alert("Erreur", ex.getMessage(), Alert.AlertType.ERROR);
        }
    ...
    private void openAddDialog() {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/fxml/add-candidature.fxml"));
    ...
    private HBox row(String icon, String key, javafx.scene.Node val) {
        Label k = new Label(icon + "  " + key + " :");
        k.getStyleClass().add("row-key");
        k.setPrefWidth(130);
        HBox r = new HBox(10, k, val);
    ...
    private VBox buildEmptyState(String icon, String title, String hint) {
        ...
        b.getChildren().addAll(i, t, h);
        return b;
    }

    private String cls(String s) {
