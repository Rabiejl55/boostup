...
public class DomaineController {
    ...
    public void initialize() {
        ...
        // Colonne Image personnalisée pour afficher l'image
        ...
        imageColumn.setCellFactory(column -> new TableCell<Domaine, String>() {
            ...
            protected void updateItem(String imagePath, boolean empty) {
                ...
                if (empty || imagePath == null || imagePath.isEmpty()) {
                    ...
                    try {
                        // Si tes images sont dans src/images/
    ...
    private void modifierDomaine() {
        ...
        try {
            ...
            controller.setDomaine(selected); // passer le domaine sélectionné
    ...
    private void diagnosticComplet() {
        try {
            ...
            // 1. Informations sur la classe
            System.out.println("Classe: " + this.getClass().getName());
            System.out.println("ClassLoader: " + this.getClass().getClassLoader());

            // 2. Où se trouve cette classe ?
            java.net.URL classLocation = this.getClass().getResource('/' + this.getClass().getName().replace('.', '/') + ".class");
            System.out.println("Emplacement de la classe: " + classLocation);

            // 3. Racine du classpath
            java.net.URL rootLocation = this.getClass().getResource("/");
            System.out.println("Racine du classpath: " + rootLocation);

            // 4. Tester tous les chemins possibles
            String[] cheminsTest = {
                    "/fxml/EditDomaineForm.fxml",
                    "fxml/EditDomaineForm.fxml",
                    "/EditDomaineForm.fxml",
                    "EditDomaineForm.fxml",
                    "/resources/fxml/EditDomaineForm.fxml",
                    "resources/fxml/EditDomaineForm.fxml"
            };

            for (String chemin : cheminsTest) {
                java.net.URL url = this.getClass().getResource(chemin);
                System.out.println("getResource(\"" + chemin + "\") = " + url);

                if (url == null) {
                    // Essayer avec ClassLoader
                    url = this.getClass().getClassLoader().getResource(chemin.startsWith("/") ? chemin.substring(1) : chemin);
                    System.out.println("  via ClassLoader: " + url);
                }
            }

            // 5. Lister tous les fichiers dans /fxml si le dossier existe
            java.net.URL fxmlDir = this.getClass().getResource("/fxml");
            if (fxmlDir != null) {
                System.out.println("\nContenu du dossier /fxml:");
                java.io.File dir = new java.io.File(fxmlDir.toURI());
                for (java.io.File file : dir.listFiles()) {
                    System.out.println("  - " + file.getName());
                }
            } else {
                System.out.println("\nLe dossier /fxml n'existe pas dans les ressources");

                // Chercher le dossier fxml ailleurs
                java.net.URL[] searchPaths = {
                        this.getClass().getResource("/"),
                        this.getClass().getClassLoader().getResource("")
                };

                for (java.net.URL path : searchPaths) {
                    if (path != null) {
                        System.out.println("Recherche dans: " + path);
                        java.io.File baseDir = new java.io.File(path.toURI());
                        chercherFichier(baseDir, "EditDomaineForm.fxml");
                    }
                }
            }
